...
/**
 * ;@@ #12 이슈 작업
 * 고정 좌표 저장 방식인 현재에서 비율로 저장으로 변경.
 * 이 경우 클릭시 현재 클라 크기에 따라 위치를 가변으로 계산해야함. | {@link HotKeyManager.GetKeyPos} 
 * wingetclientpos 함수 사용.
 * 가상키랑 오버레이 두 부분에서 사용하는 keyData.pos 부분을 확인 필요.
 */
